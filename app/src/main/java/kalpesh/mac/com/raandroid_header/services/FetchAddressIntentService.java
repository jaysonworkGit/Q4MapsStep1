...
public class FetchAddressIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        ArrayList<String> addressText  = intent.getStringArrayListExtra(Constants.LOCATION_LIST_EXTRA);

       // addressText.add("nw100tn");
       // addressText.add("paris");

        for(int ipos = 0; ipos < addressText.size() ; ipos++) {
        ...
        try {
            ...
            addresses = geocoder.getFromLocationName(addressText.get(ipos),1);
        ...
        if (addresses == null || addresses.size()  == 0) {
            ...
            String currentLocation = String.valueOf(address.getLatitude()) + " " + String.valueOf(address.getLongitude()) ;

                for (int i = 0; i < address.getMaxAddressLineIndex(); i++) {
                    if(  !currentLocation.equals(copyCheck) )  { // some times there is more than one address in here I do not want the same thing twice in a row.
